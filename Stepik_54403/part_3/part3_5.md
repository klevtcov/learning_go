...
```
reader := csv.NewReader("объект удовлетворяющий интерфейсу io.ReadWriter")
reader.Comma = ';'    // указываем новый символ разделитель, по умолчанию ,
reader.Comment = '*'  // указываем новый символ комментария, по умолчанию #
```
...
Кст, чтобы не было спуска внутрь директории, то нужно вернуть filepath.SkipDir вместо nil 
```
